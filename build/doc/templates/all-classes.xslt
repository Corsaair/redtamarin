			...
			<td>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:if test="prolog/asMetadata/apiVersion/apiPlatform[@name='AVM'] and not (prolog/asMetadata/apiVersion/apiPlatform[@name='Flash'])">
							<xsl:value-of select="$nbsp"/>
							<img src="images/AVMIcon12x12.png" width="12" height="12" hspace="0" vspace="0"/>
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
			</td>
