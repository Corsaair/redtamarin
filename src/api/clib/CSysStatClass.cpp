...
namespace avmshell
{
    ...
    int CSysStatClass::_mkdir(Stringp path, int mode)
    {
        if (!path) {
    ...
    /*static*/ int CSysStatClass::stat(ScriptObject* self, Stringp path, CStatusObject* buf)
    {
        ...
        if( result != -1 )
        {
            ...
            buf->set_st_size( statbuf.st_size );
            buf->set_st_atime( statbuf.st_atime );
            buf->set_st_mtime( statbuf.st_mtime );
            buf->set_st_ctime( statbuf.st_ctime );
        }
